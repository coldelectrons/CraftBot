...
Status GetAllChestsAround(BehaviourClient& c) {
  ...
  Position checked_position;
  {
    ...
    const Block* block;
    const std::map<std::pair<int, int>, std::shared_ptr<Chunk>>& all_chunks =
        world->GetAllChunks();
      ...
      for (int x = 0; x < CHUNK_WIDTH; ++x) {
        ...
        for (int y = 0; y < world->GetHeight(); ++y) {
          ...
          for (int z = 0; z < CHUNK_WIDTH; ++z) {
            ...
            if (block &&
                block->GetBlockstate()->GetName() == "minecraft:chest") {
...
Status GetSomeFood(BehaviourClient& c, const std::string& food_name) {
  ...
  const std::vector<Position>& chests =
      c.GetBlackboard().Get<std::vector<Position>>("World.ChestsPos");
  ...
  std::mt19937 random_engine = std::mt19937(static_cast<unsigned int>(
  ...
  for (size_t index = 0; index < chests.size(); ++index) {
    ...
    while (true) {
      std::vector<short> slots_src;
      {
        ...
        for (auto it = slots.begin(); it != slots.end(); ++it) {
          // Chest is src
          if (it->first >= 0 && it->first < first_player_index &&
    ...
    while (
        ...
        AssetsManager::getInstance()
            .Items()
            .at(inventory_manager->GetPlayerInventory()
      ...
      if (std::chrono::duration_cast<std::chrono::milliseconds>(
              ...
              .count() >= 10000) {
        LOG_WARNING(
            "Something went wrong trying to get food from chest (Timeout).");
    ...
    if (it->first >= Window::INVENTORY_STORAGE_START &&
      ...
      blocks_in_inventory.insert(
          AssetsManager::getInstance()
              ...
              ->GetName());
#else
      blocks_in_inventory.insert(AssetsManager::getInstance()
                                     .Items()
                                     .at(it->second.GetItemID())
                                     ->GetName());
#endif
...
Status SwapChestsInventory(BehaviourClient& c,
                           const std::string& food_name,
                           const bool take_from_chest) {
  std::shared_ptr<InventoryManager> inventory_manager = c.GetInventoryManager();
  ...
  while (true) {
    ...
    // Select a chest
    size_t chest_index_index =
        chest_indices.size() == 1
            ? 0
            : std::uniform_int_distribution<int>(
                  0, static_cast<int>(chest_indices.size()) - 1)(random_engine);
    ...
    // Find possible swaps
    {
      ...
      for (auto it = slots.begin(); it != slots.end(); ++it) {
        ...
        if (it->first >= 0 && it->first < first_player_index &&
            take_from_chest && !it->second.IsEmptySlot()
#if PROTOCOL_VERSION < 347
            &&
            AssetsManager::getInstance()
                    .Items()
        ...
        else if (it->first >= first_player_index && take_from_chest &&
                 it->second.IsEmptySlot()) {
          slots_dst.push_back(it->first);
        }
        // If !take, chest is dst
        else if (it->first >= 0 && it->first < first_player_index &&
                 !take_from_chest && it->second.IsEmptySlot()) {
          slots_dst.push_back(it->first);
        }
    ...
    int dst_index = -1;
    int src_index = -1;
    if (slots_src.size() > 0 && slots_dst.size() > 0) {
      // Select a random slot in both src and dst
      dst_index =
          slots_dst.size() == 1
              ? 0
              : std::uniform_int_distribution<int>(
                    0, static_cast<int>(slots_dst.size()) - 1)(random_engine);
      src_index =
          slots_src.size() == 1
              ? 0
              : std::uniform_int_distribution<int>(
      ...
      // Try to swap the items
      swap_success = SwapItemsInContainer(c, container_id, slots_src[src_index],
                                          slots_dst[dst_index]);
    }
    ...
    auto start = std::chrono::steady_clock::now();
    const short checked_slot_index =
        (take_from_chest ? slots_dst[dst_index] : slots_src[src_index]) -
        first_player_index + Window::INVENTORY_STORAGE_START;
    while (true) {
      if (std::chrono::duration_cast<std::chrono::milliseconds>(
              std::chrono::steady_clock::now() - start)
              .count() >= 10000) {
        LOG_WARNING(
            "Something went wrong trying to get items from chest (Timeout).");
        return Status::Failure;
      }
      {
        std::lock_guard<std::mutex> inventory_lock(
            inventory_manager->GetMutex());
        const Slot& slot = inventory_manager->GetPlayerInventory()->GetSlot(
            checked_slot_index);
...
Status FindNextTask(BehaviourClient& c) {
  ...
  const std::vector<std::vector<std::vector<short>>>& target =
      blackboard.Get<std::vector<std::vector<std::vector<short>>>>(
          "Structure.target");
  const std::map<short, std::string>& palette =
      blackboard.Get<std::map<short, std::string>>("Structure.palette");

  const std::set<std::string>& available =
      blackboard.Get<std::set<std::string>>("Inventory.block_list");
  ...
  start_pos.x = std::min(
      end.x, std::max(start.x, static_cast<int>(std::floor(
                                   entity_manager->GetLocalPlayer()->GetX()))));
  start_pos.y = std::min(
      end.y, std::max(start.y, static_cast<int>(std::floor(
                                   entity_manager->GetLocalPlayer()->GetY()))));
  start_pos.z = std::min(
      end.z, std::max(start.z, static_cast<int>(std::floor(
                                   entity_manager->GetLocalPlayer()->GetZ()))));
  ...
  while (!to_explore.empty()) {
    ...
    for (auto it = to_explore.begin(); it != to_explore.end(); ++it) {
      ...
      const int target_palette =
          target[pos.x - start.x][pos.y - start.y][pos.z - start.z];
...
#if PROTOCOL_VERSION < 347
          blockstate =
              AssetsManager::getInstance().Blockstates().at(0).at(0).get();
#else
      ...
      else if ((target_palette != -1 && !blockstate->IsAir() &&
                target_name != blockstate->GetName()) ||
    ...
    if (pos_candidates.size() > 0) {
      ...
      std::vector<Vector3<double>> other_player_pos;
      {
        ...
        for (auto it = entity_manager->GetEntities().begin();
             it != entity_manager->GetEntities().end(); ++it) {
      ...
      int selected_index =
          max_dist_indices.size() == 1
              ? 0
              : max_dist_indices[std::uniform_int_distribution<int>(
                    0, static_cast<int>(max_dist_indices.size()) -
                           1)(random_engine)];

      blackboard.Set<std::string>(
          "NextTask.action",
          item_candidates[selected_index].empty() ? "Dig" : "Place");
...
Status CheckCompletion(BehaviourClient& c) {
  ...
  const bool log_details =
      blackboard.Get<bool>("CheckCompletion.log_details", false);
  const bool log_errors =
      blackboard.Get<bool>("CheckCompletion.log_errors", false);
  const bool full_check =
      blackboard.Get<bool>("CheckCompletion.full_check", false);
      ...
      for (int z = start.z; z <= end.z; ++z) {
        ...
        const Blockstate* blockstate;
        {
          ...
          if (!block) {
            if (target_id != -1) {
              ...
              if (log_details &&
                  missing_blocks <
                      100)  // Don't print more than 100 missing blocks
              {
                LOG_INFO("Missing " << palette.at(target_id) << " in "
                                    << world_pos);
              }
            }
            continue;
        ...
        if (target_id == -1) {
          if (!blockstate->IsAir()) {
            ...
            additional_blocks++;
            if (log_details) {
              LOG_INFO("Additional " << blockstate->GetName() << " in "
                                     << world_pos);
            }
          }
        } else {
          if (blockstate->IsAir()) {
            ...
            if (blockstate->GetName() != target_name) {
              ...
              wrong_blocks++;
              if (log_details) {
                LOG_INFO("Wrong " << blockstate->GetName() << " instead of "
                                  << target_name << " in " << world_pos);
              }
            }
          }
        }
...
Status LoadNBT(BehaviourClient& c,
  ...
  try {
    ...
    LOG_ERROR(
        "Error loading NBT file. Make sure the file is uncompressed (you can "
        "change the extension to .zip and simply unzip it)");
    return Status::Failure;
  }
  ...
  const std::vector<NBT::TagCompound>& palette_list =
      loaded_file["palette"].as_list_of<NBT::TagCompound>();
  ...
  Position min(std::numeric_limits<int>().max(),
               std::numeric_limits<int>().max(),
               std::numeric_limits<int>().max());
  Position max(std::numeric_limits<int>().min(),
  ...
  const std::vector<NBT::TagCompound>& block_tag =
      loaded_file["blocks"].as_list_of<NBT::TagCompound>();
  ...
  std::vector<std::vector<std::vector<short>>> target(
      size.x,
      std::vector<std::vector<short>>(size.y, std::vector<short>(size.z, -1)));
  ...
  if (id_temp_block == -1) {
    LOG_WARNING("Can't find the given temp block " << temp_block
                                                   << " in the palette");
  } else {
    ...
    if (log_info) {
      LOG_INFO("Removed the bottom " << removed_layers << " layer"
                                     << (removed_layers > 1 ? "s" : ""));
    }
  }
...
Botcraft::Status CreateCircle(Botcraft::BehaviourClient& c,
                              const Botcraft::Position& center,
                              float radius,
                              float thickness,
                              bool outwards,
                              float height,
  ...
  // XXX TODO double check the outcome of integer dimensions since I also
  // using float params
  // XXX WTF is start >= end?
  Position start;
  Position end;
  if (outwards) {
    start = Position(center.x + radius + thickness, center.y + height,
                     center.z + radius + thickness);
    end = Position(center.x - radius - thickness, center.y,
                   center.z - radius - thickness);
  } else {
    start = Position(center.x + radius, center.y + height, center.z + radius);
    end = Position(center.x - radius, center.y, center.z - radius);
  }
  Position max(std::floor(start.x), std::floor(start.y), std::floor(start.z));
  Position min(std::floor(end.x), std::floor(end.y), std::floor(end.z));
  ...
  // Fill the target area with air (-1)
  // TODO probably should incorporate minecraft:structure_void for NOT_CHANGE
  // XXX isn't there some better multidimensional array to use?
  std::vector<std::vector<std::vector<short>>> target(
      size.x,
      std::vector<std::vector<short>>(size.y, std::vector<short>(size.z, -1)));

  // Compute all block to place
  // loop z
  //   loop x
  //     test d
  //       loop y, add block
  auto sd_min = std::pow(outwards ? radius : radius - thickness, 2);
  auto sd_max = std::pow(outwards ? radius + thickness : radius, 2);
  for (auto x = start.x; x < end.x; ++x) {
    for (auto z = start.z; z < end.z; ++z) {
      auto sd = std::pow(x + 0.5, 2.0) + std::pow(z + 0.5, 2.0);
      if (sd_min <= sd and sd <= sd_max) {
  ...
  // if (id_temp_block == -1)
  // {
  //     LOG_WARNING("Can't find the given temp block " << temp_block << " in
  //     the palette");
  // }
